...
int main() {
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		//Ejercicio 1: Hacer la tabla para la mesa
	    model = glm::mat4(1.0f);
		...
		//Ejercicio 2: Agregar las 4 patas de la mesa
		//Pata 1:
		model = glm::mat4(1.0f);
		model = glm::translate(model, glm::vec3(0.5f, -0.1f, 0.5f));
		model = glm::scale(model, glm::vec3(0.1f, 1.3f, 0.1f));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		glDrawArrays(GL_TRIANGLES, 0, 36);
 ...
 void Inputs(GLFWwindow *window) {
	 ...
	 if (glfwGetKey(window, GLFW_KEY_LEFT) == GLFW_PRESS)
		 rotY -= 0.4f;
	 //Ejercicio 3: Transformaciones de Vista
