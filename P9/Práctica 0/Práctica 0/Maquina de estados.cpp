...
void Animation() {
	...
	if (AnimDog)
	{
		...
		dogPos += glm::vec3(0.0f, 0.0f, 0.0001f);
		
		dogAnim = 3;
		FLegs = 10.0f * sin(glfwGetTime() * dogAnim);
		RLegs = 10.0f * sin(glfwGetTime() * dogAnim + glm::radians(180.0f));
		tail = 10.0f * sin(glfwGetTime() * dogAnim);
		head = 10.0f * sin(glfwGetTime() * dogAnim);
	}
